            ...
            <div className="container mx-auto px-4 py-8">
                    ...
                    <div className="text-lg text-gray-300 sm:w-2/3">
                        <div className="flex flex-wrap gap-2 mb-4">
                            {anime.data.genres.map((genre, index) => (
                                <Link href={genre.url} key={index} className="text-xl bg-white text-black px-3 py-1 rounded-md shadow hover:bg-sky-500 hover:text-white transition ease-in-out delay-150 hover:translate-y-1">
                                    {genre.name}
                                </Link>
                            ))}
                        </div>
                ...
                {anime.data.trailer.youtube_id && (
                    <div className="py-8 flex justify-center">
                        <VideoPlayer videoId={anime.data.trailer.youtube_id} className="rounded-lg overflow-hidden shadow-xl w-full sm:w-3/4" />
                    </div>
                )}
            </div>
        </div>
